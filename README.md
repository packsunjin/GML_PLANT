...
#### ⚠️ 수집 중에는 대시보드를 끄세요 (또는 웹의 '측정 시작'으로 수집)

ADS1115는 **ADC가 하나뿐**이고 mux로 채널을 돌려씁니다. A0(전위)를 250Hz로 읽는 도중에
누가 A1(토양수분)을 읽으면 mux가 전환되면서 **A0 스트림에 다른 채널 값(주로 0V)이 섞입니다.**

실제로 이 때문에 168ms마다 2샘플씩 정확히 0V로 떨어지는 데이터가 기록된 적이 있습니다.
스펙트럼에는 6Hz와 그 배수(12/18/24/30/36Hz)가 줄줄이 나타나는데, 이건 **강하가 만든
하모닉**이지 식물 신호가 아닙니다.

대응:

- 수집 중에는 `SUSPEND_AUX` 로 **A1 읽기를 자동으로 멈춥니다**(같은 프로세스 안).
- ADS1115 접근을 락으로 **직렬화**합니다.
- **다른 프로세스는 막을 수 없습니다.** SSH로 `sensor_control.py`를 돌리면서 웹
  대시보드가 같이 켜져 있으면 두 프로세스가 같은 ADC를 번갈아 읽습니다.
  → **웹의 `측정 시작` 으로 수집하거나**(실시간 루프를 자동으로 재웁니다),
    대시보드를 끄고 수집하세요.
- 수집이 끝나면 **자동으로 검사해서 알려줍니다**:

```
[sensor_control] ⚠️  주기적인 0V 강하가 발견됐습니다 (12.0Hz = 83ms 간격, 전체의 1.4%, 봉우리 962배).
    식물 신호가 아니라 ADC 읽기 아티팩트입니다.
```

> 판정은 "0V에 붙은 샘플"을 0/1 신호로 만들어 **스펙트럼에 뾰족한 봉우리가 있는지**로 합니다.
> 간격 통계는 깊은 강하만 잡힐 때 한 주기/두 주기가 섞여 무너지기 때문입니다.
> 불규칙한 스파이크(진짜 자극 반응)에는 반응하지 않습니다.
