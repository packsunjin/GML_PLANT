...
### 8. 웹에서 데이터 수집 · 재학습 (`/api/collect`, `/api/train`, `/api/pipeline`)
...
**전체 자동 (`/api/pipeline`)** — 버튼 하나로 아래를 순서대로 다 돌립니다.

```
1-1 '정상' 수집 (30초)      → data/raw/정상.csv
1-2 '수분부족' 수집 (30초)  → data/raw/수분부족.csv
1-3 '자극' 수집 (30초)      → data/raw/자극.csv
2   전처리                   → 필터 → 스펙트로그램 PNG → 특징 14개
3   학습                     → models/*.joblib + 혼동행렬
↻   새 모델을 돌고 있는 분류기에 바로 적용
```

- 기존 원시 데이터를 덮어쓰므로 **확인창**을 거칩니다.
- 중간에 **`■ 중지`** 를 누르면 그 단계에서 멈춥니다(수집 중이었다면 모은 데이터는 저장).
- 학습 단계에서 중지하면 모델이 반쯤 저장됐을 수 있어 **새 모델을 적용하지 않습니다.**

```bash
curl -X POST http://<파이IP>:5000/api/pipeline \
     -H "Content-Type: application/json" \
     -d '{"duration":30,"task":"3종","mode":"둘다"}'

# 수집은 건너뛰고 이미 있는 data/raw 로 전처리+학습만
curl -X POST http://<파이IP>:5000/api/pipeline \
