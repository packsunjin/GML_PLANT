...
### 7. 웹에서 데이터 수집 · 재학습 (`/api/collect`, `/api/train`)

대시보드 오른쪽 위 **`수집 · 학습`** 버튼을 누르면 패널이 열립니다. **SSH 없이 브라우저에서**
데이터 수집부터 재학습, 새 모델 적용까지 전부 됩니다.

```
1 · 데이터 수집    [정상] [수분부족] [자극]   길이 [30] 초
2 · 재학습         [3종 ▾] [둘다 ▾]  [전처리 + 학습 시작]
   ── 실행 로그가 아래에 실시간으로 흐름 ──
```
...
- 이미지는 **눌러서 크게** 볼 수 있고, 모든 파일에 **내려받기** 링크가 있습니다.
- 체크박스로 고른 뒤 **`선택 삭제`** — 확인창을 거칩니다. 되돌릴 수 없습니다.
- **지금 추론에 쓰고 있는 모델**은 `사용 중`으로 표시되고 삭제가 막힙니다.
