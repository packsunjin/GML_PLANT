...
> 확신도 링, 파형/스펙트로그램 확대 팝업, 카드·그림자·알약버튼 디자인은
> 분류 판단에 쓰이지 않는 장식 요소라 화면에서 뺐습니다. 스펙트로그램은
> `3 · 자료` 탭에서 학습에 쓴 실제 이미지를 볼 수 있고, 온·습도는 `센서` 탭의
> 진단하기로 확인합니다. (`/data` 응답에서도 `spec`/`temp`/`humidity` 필드를
> 더 이상 보내지 않습니다 — 분류 자체는 서버 내부에서 그대로 spec을 계산해 씁니다.)
