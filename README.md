...
> 무엇을 잰 값인지(`AHT20` / `토양수분(A1)`)는 `센서` 탭의 `진단하기` 로그에 표시됩니다
> (메인 화면에는 온·습도 게이지가 없습니다 — 위 "초록말 UI" 참고).
...
| 토양수분 보정 · **모델 전환**(`사용하기`) | 자료 목록 보기 · 내려받기 · 센서 진단 |
...
> ⚠️ **주의**: 겉보기엔 똑같은 "주기적인 강하" 패턴이 **전극 임피던스가 아니라
> ADC 채널 혼선**(바로 아래 "수집 중 ADC 채널 혼선" 항목 참고) 때문에
> 생길 수도 있습니다. 실제로 이 프로젝트에서 겪은 강하 중 하나는 전극을 전혀
> 안 바꿨는데도 `SUSPEND_AUX` 수정 이후 0%로 사라졌습니다 — 원인이 전극이 아니라
> 웹 대시보드의 온·습도 스레드가 수집 중에도 A1을 읽어버린 소프트웨어 문제였던
> 것입니다. **그러니 강하가 보이면 전극부터 바꾸지 말고, 먼저 웹의 `측정 시작`으로
> 수집했는지(SSH에서 `sensor_control.py`를 따로 돌리지 않았는지) 확인하세요.**
...
위 두 항목(fast-restore, ADC 채널 혼선)은 RA/LA(측정 전극) 임피던스나 소프트웨어 문제였습니다.
...
> fast-restore/ADC 채널 혼선과 마찬가지로, 이 문제도 **아날로그 프런트엔드(RLD 루프)** 문제라
...
### 11. 지금 쓰는 모델 바로 바꾸기 (`/api/model/activate`)
...
> 온·습도 게이지, 실시간 스펙트로그램, 확신도 추이 그래프, 상태 변화 기록,
> 파형/스펙트로그램 확대 팝업은 분류 판단에 쓰이지 않는 장식 요소라 화면에서 뺐습니다.
> 스펙트로그램은 `3 · 자료` 탭에서 학습에 쓴 실제 이미지를 볼 수 있고, 온·습도는
> `센서` 탭의 진단하기로 확인합니다. (`/data` 응답에서도 `spec`/`temp`/`humidity`
> 필드를 더 이상 보내지 않습니다 — 분류 자체는 서버 내부에서 그대로 spec을 계산해 씁니다.)
