...
그런데 노치 필터 기본값은 **50Hz**(유럽/시뮬레이션 기준)라 한국에서는 **아무 일도 하지
않았습니다.** 그래서 신호에서 50Hz와 60Hz의 파워를 직접 재서 **센 쪽으로 자동 보정**합니다.
...
- `--notch 0` 으로 끄거나 `--notch 60` 으로 고정할 수 있습니다.

> 다행히 대역통과(0.5~45Hz)가 60Hz를 이미 대부분 잘라냅니다(60Hz 성분이 29.7% → 0.4%).
> 노치를 맞추면 0.0%가 됩니다. 상한을 45Hz보다 올릴 계획이라면 노치가 중요해집니다.
...
2. 포일과 식물 사이에 **소금물 적신 휴지**를 한 겹 끼웁니다(전해질 역할).
3. 포일 위에 **집게 클립**을 물리고 AD8232의 RA / LA에 연결합니다.
4. RL은 **젖은 흙에 3cm 이상** 찔러 넣습니다.
5. 마르지 않게 **랩으로 감싸고**, 선은 테이프로 고정합니다(움직이면 오프셋이 생겨
...
A1(토양수분)을 읽으면 A0 스트림에 다른 채널 값이 섞일 수 있습니다.

- 수집 중에는 `SUSPEND_AUX` 로 **A1 읽기를 자동으로 멈춥니다**(같은 프로세스 안).
- ADS1115 접근을 락으로 **직렬화**합니다.
- 다른 프로세스는 막을 수 없으므로, **웹의 `측정 시작` 으로 수집**하는 것이 안전합니다
  (실시간 루프를 자동으로 재웁니다).
...
반응 스파이크가 보통 창(2초) 안에서 수십 ms만 튀기 때문에 std로는 나머지 조용한
