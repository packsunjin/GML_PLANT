...
스펙트로그램 이미지 기반 머신러닝(SVM/Random Forest)으로 **정상 / 수분부족 / 자극 / 꺾임** 4가지
상태를 실시간으로 분류하여 VNC 또는 브라우저(웹 대시보드)로 원격 모니터링하는 시스템입니다.
- **정상 · 수분부족** = 지속 상태(모니터링 대상), **자극** = 순간 이벤트(접촉/충격), **꺾임** = 물리적 손상(순간 상처 후 지속)
- 기본은 **4종 분류**이며, `정상-수분부족`·`정상-자극`·`정상-꺾임`·`자극-꺾임` 같은 **2종 비교**도 지원합니다.
...
```
project/
  data/raw/          # (1) 원시 시계열 CSV (정상.csv, 수분부족.csv, 자극.csv, 꺾임.csv 중 수집된 것만)
  data/spectrogram/  # (2) 전처리된 224x224 스펙트로그램 이미지 (정상/, 수분부족/, 자극/, 꺾임/)
...
| 옵션 | 값 | 의미 |
|---|---|---|
| **`--task`** | `4종` (기본) | 정상 vs 수분부족 vs 자극 vs 꺾임 → `models/best_model.joblib` |
| | `정상-수분부족` | 정상 vs 수분부족 → `models/best_model_정상-수분부족.joblib` |
| | `정상-자극` | 정상 vs 자극 → `models/best_model_정상-자극.joblib` |
| | `정상-꺾임` | 정상 vs 꺾임 → `models/best_model_정상-꺾임.joblib` |
| | `자극-꺾임` | 자극 vs 꺾임 (기계적 자극 둘이 구분되나 테스트) → `models/best_model_자극-꺾임.joblib` |
| | `전체` | 위 5과제 전부 학습·비교 |
...
| **`--sim_state`** | `정상`(기본)/`수분부족`/`자극`/`꺾임`/`순환` | 하드웨어·CSV 없을 때 라이브로 생성할 상태(`순환`=8초마다 순환) |
...
python3 main.py --no-gui --sim_state 순환             # 정상→수분부족→자극→꺾임 순환 데모
```
...
>    - `순환` → **8초마다** 정상 → 수분부족 → 자극 → 꺾임 순으로 바꿔가며 생성 (센서·파일 없이 네 상태를 다 보여주는 **시연용**)
>
...
python3 sensor_control.py --state 꺾임 --duration 30 --rate 250
```
...
- `--task` (기본 `4종`): 어떤 상태들을 분류할지.
  - `4종`(기본): 정상 vs 수분부족 vs 자극 vs 꺾임 → `models/best_model.joblib` (실시간 기본 모델)
  - `정상-수분부족` / `정상-자극` / `정상-꺾임`: 정상 vs 각 상태 (2종)
  - `자극-꺾임`: 두 기계적 자극(순간 접촉 vs 지속 손상)이 전기신호로 구분되는지 테스트
  - `전체`: 위 5과제 전부 학습·비교
- `--mode` (기본 `픽셀`): `픽셀` / `특징` / `둘다`
...
python3 train.py --task 정상-꺾임 --mode 둘다     # 정상 vs 꺾임 2종 비교
...
|---|---|---|---|
| **4종 (정상/수분부족/자극/꺾임)** | 픽셀 | RandomForest | 0.8611 |
| **4종 (정상/수분부족/자극/꺾임)** | 특징 | RandomForest | 0.8056 |
| 정상-수분부족 / 정상-자극 / 정상-꺾임 / 자극-꺾임 | 픽셀·특징 | SVM | 1.0000 |
...
`web/` 폴더의 디자인 화면을 Flask가 그대로 서빙합니다. `--ui 기본`을 주면 예전 내장 화면으로 돌아갑니다.

| 파일 | 역할 |
|---|---|
| `web/chorokmal.html` | 7개 화면(홈·실시간 상태·케어 랭킹·상태 기록·내 식물·최저가 마켓·설정) |
| `web/static/app.css` | 디자인 시스템 스타일 |
| `web/static/ui.css` | 화면 전환·상태색 등 보정 스타일 |
| `web/static/live.js` | `/data` 폴링 + 화면 갱신 |
| `web/static/img/` | 식물 사진 |

**실시간으로 갱신되는 것은 "실시간 상태" 화면**입니다. `live.js`가 0.7초마다 `/data`를 읽어
아래를 다시 그립니다(나머지 화면은 아직 고정된 예시 화면입니다).

- 분류 결과 이름 · 확신도 · 확신도 링(바깥 얇은 선)
- 상태별 확률 4종 막대(정상/수분부족/자극/꺾임)
- 전위신호 파형, 스펙트로그램 (`펼치기 ⤢` 로 팝업 확대)
- 입력 소스 배지(하드웨어/시뮬레이션/CSV)
- 상태에 따라 색이 함께 바뀝니다: 정상 초록 / 수분부족 파랑 / 자극 주황 / 꺾임 빨강

> 온도·습도 게이지는 아직 **표시용 고정값**입니다. 온·습도 센서를 붙이면 `_payload()`에 값을
> 추가하고 `live.js`에서 막대 위치만 연결하면 됩니다.

#### 기록 기능 (하드웨어 없이도 동작)

`src/store.py`가 기록을 파일로 남깁니다. 데이터베이스 없이 파일만 쓰므로 파이 없이
노트북에서 시뮬레이션으로 돌려도 그대로 쌓입니다.

```
data/app/events.jsonl   상태 변화·물주기 이벤트 (한 줄에 JSON 하나)
data/app/plants.json    등록한 식물 목록
data/app/settings.json  알림 설정
```

- 백그라운드 분류기가 **상태가 바뀔 때만** 기록합니다(같은 상태 반복은 10분 안에는 생략).
- **상태 기록** 화면이 이 기록을 읽어 일별 막대·기간 요약·이벤트 타임라인을 그립니다
  (7일 / 14일 / 30일 전환).
- **물주기 기록하기** 버튼(홈·상태 기록)을 누르면 기록되고 타임라인에 바로 반영됩니다.
- **홈 화면**의 "현재 식물의 마음" 문구와 파형도 실시간 분류 결과를 따라 바뀝니다.

| API | 설명 |
|---|---|
| `GET /data` | 최신 분류 결과 (실시간 화면용) |
| `GET /api/history?days=7` | 일별 요약 + 최근 이벤트 + 누적 통계 |
...
- **꺾임(물리적 손상) 상태 추가 → 4종 분류**: 자극(순간 이벤트)과 별개로 지속되는 손상 상태 `꺾임`을 추가해 **정상/수분부족/자극/꺾임 4종**으로 확장. `자극-꺾임`(두 기계적 자극 구분) 등 비교 과제도 추가. 4종은 정확도가 100%가 아니라 ~86%로, 클래스가 늘며 서로 헷갈리는 게 생기는 현실적인 결과가 나옵니다.
