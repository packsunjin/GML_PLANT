...
**센서가 안 잡힐 때** — 대시보드 `수집 · 학습` 패널 맨 아래 **`진단하기`** 버튼을 누르면
...
대시보드 오른쪽 위 **`수집 · 학습`** 버튼을 누르면 패널이 열립니다. **SSH 없이 브라우저에서**
데이터 수집부터 스펙트로그램 변환, 학습, 새 모델 적용까지 전부 됩니다.

```
1 · 데이터 수집        [정상] [수분부족] [자극]  길이 [30] 초  □ 중지할 때까지
2 · 스펙트로그램 변환  [정상] [수분부족] [자극] [전체]
3 · 학습               [3종 ▾] [둘다 ▾]  [학습 시작]
센서                   [진단하기] [지금이 젖음(100%)] [지금이 마름(0%)]
   ── 실행 로그가 아래에 실시간으로 흐름 ──
```
...
| 단계 | 버튼 | 하는 일 |
|---|---|---|
| 1 · 데이터 수집 | `정상` `수분부족` `자극` | 상태별로 하나씩 모아 `data/raw/<상태>.csv` |
| 2 · 스펙트로그램 변환 | `정상` `수분부족` `자극` `전체` | 상태별로 따로 변환 (`preprocess.py --only`) |
| 3 · 학습 | `학습 시작` | 변환된 **전체** 데이터로 한 번에 학습 |
...
```bash
curl -X POST http://<파이IP>:5000/api/preprocess \
     -H "Content-Type: application/json" -d '{"state":"자극"}'   # 하나만
curl -X POST http://<파이IP>:5000/api/preprocess -d '{}'          # 전체
...
- **수집**: 상태 버튼을 누르면 `sensor_control.py`가 그 길이만큼 돌아 `data/raw/<상태>.csv`에
  저장합니다. 이미 수집한 상태는 버튼에 `✓`가 붙고, 다시 누르면 덮어씁니다.
...
헤더의 **`학습 자료`** 버튼을 누르면 학습에 쓰인 파일을 브라우저에서 직접 다룰 수 있습니다.

| 탭 | 내용 |
|---|---|
| 원시 데이터 (CSV) | `data/raw/*.csv` — 수집한 전위 신호 |
| 특징 테이블 (CSV) | `data/features.csv` — 윈도우별 특징 14개 |
| 스펙트로그램 이미지 | `data/spectrogram/<상태>/*.png` — 학습에 쓰인 224×224 |
