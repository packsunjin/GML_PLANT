# GML_PLANT — 식물 전기신호 실시간 상태 분류 시스템

> **머신러닝 모델 기반의 식물 전기 신호 시각화 및 패턴 분석을 활용한 실시간 상태 분류 시스템 구현**
> 제6회 학생 SW·AI 인재 양성 프로젝트 · 팀 **Green Mind Lab**
> 영천고등학교 1학년 박선진 · 유도혁 · 이유찬 · 박소미 | 지도교사 이민정 | 자문위원 ㈜에이포랩 김민재

AD8232(생체전위 증폭기) + ADS1115(16비트 ADC)로 방울토마토 잎·줄기의 미세 전위를 250 Hz로
측정하고, 스펙트로그램과 명시적 특징 14개를 뽑아 SVM·Random Forest로 **정상 / 수분부족 / 자극**
세 상태를 분류하는 것을 목표로 한 시스템입니다. 수집–전처리–학습–실시간 추론 전 과정을
브라우저 하나로 돌릴 수 있습니다.

### 지금 어디까지 됐나 (솔직한 상태)

측정·학습 **시스템은 완성**했지만, **분류 목표는 아직 검증되지 않았습니다.**

| | 상태 |
|---|---|
| 브라우저 통합 시스템(수집–전처리–학습–추론) | ✅ 동작 |
| 측정계 결함 4종 발견·제거 | ✅ 완료 |
| 정상·자극 2상태 분류 | ⚠️ 분류는 되지만 **그 신호가 식물에서 왔는지 미검증** |
| 수분부족 포함 3상태 분류 | ❌ 현 장비로 **원리적으로 측정 불가** |

- **왜 미검증인가** — 접촉할 때 잡히는 반응이 식물 반응인지, 배선 진동이나 사람 몸을 타고
  들어온 전위인지 가르는 대조실험을 아직 못 했습니다. 설계는 끝났고, 알루미늄 호일만 있으면
  10분이면 되는 것부터 있습니다 (아래 **「먼저 해야 할 실험 — 호일 대조군」**).
- **왜 수분부족이 불가능한가** — 수분 스트레스는 수 시간 규모(≈10⁻⁴ Hz)인데 AD8232의
  0.5 Hz 고역통과 필터가 이 대역을 통째로 지웁니다. 소프트웨어로는 복구할 수 없습니다
  ([구조적 한계](#ad8232로-잴-수-있는-신호의-크기와-속도--이게-구조적-한계입니다)).

이 한계를 대역·임피던스 수준에서 정량 분석한 것이 본 연구의 주된 결과이며,
전체 내용은 [`report/`](report/)의 결과보고서에 있습니다.
...
src/
  sensor_control.py      AD8232+ADS1115 수집·안정화 대기·드리프트 점검
  preprocess.py          대역통과·노치 필터, 레일 포화 게이트, 스펙트로그램 생성
  spectro_render.py      학습과 추론이 공유하는 스펙트로그램 변환
  feature_extraction.py  시간·주파수 영역 특징 14개
  train.py               학습·평가 (시간순 그룹 분할로 데이터 누수 차단)
  inference.py           실시간 추론
  gui.py                 GUI / 헤드리스 화면
  web_dashboard.py       브라우저 통합 화면
  check_sensors.py       센서가 왜 안 잡히는지 진단
  sanity_check.py        분석 절차 자체를 검증 (누수·우연 정확도)
  venv_boot.py           venv 를 안 켜도 실행되게
  font_utils.py          그래프의 한글 폰트 찾기
report/                  결과보고서 원고(tpl.html)와 DOCX 빌드 도구
deploy/                  전원만 꽂으면 자동 실행되게 하는 systemd 등록
data/ · models/          수집한 CSV · 학습된 모델
```

학습과 실시간 추론이 `spectro_render.py`·`feature_extraction.py`를 그대로 공유하므로,
학습할 때와 서빙할 때 처리가 달라지는 문제(train/serve skew)가 구조적으로 생기지 않습니다.
...
##### 전극 접촉이 나쁘면 왜 레일에 붙는가 — 숫자로

"임피던스가 높으면 신호가 전압 분배로 사라진다"는 설명을 봤을 수 있는데, **AD8232에는
해당하지 않습니다.** 데이터시트상 차동 입력 임피던스가 **10 GΩ**이라, 전극이 1 GΩ이어도
직류 손실은 9%뿐입니다.

실제로 문제를 일으키는 건 다른 넷입니다.

| 기전 | 전극 1 GΩ일 때 | 왜 문제인가 |
|---|---|---|
| **오프셋 전류 × 전극 저항** | **25 mV** | 허용 입력 ±1.23 mV의 **20배** → 레일 |
| 입력 용량과 저역통과 | 차단 **1.1 Hz** | 분석 대역(0.5–20Hz) 한복판이 깎임 |
| CMRR 균형 붕괴 | — | 60Hz가 차동으로 변환되어 증폭됨 |
| 전극 저항의 열잡음 | 17.9 µV | 문헌의 1mV 미만 신호와 견줄 크기 |

첫 번째가 핵심입니다. AD8232의 입력 오프셋 전류는 25 pA(typ)인데, 이게 전극 저항을
지나며 전압을 만듭니다. 1 GΩ이면 25 mV — 잴 수 있는 범위를 20배 넘습니다.

0.5Hz 고역통과가 이 직류를 상쇄해 주지만, **접촉이 조금만 흔들리면** 오프셋이 범위를
벗어나 증폭기가 레일에 닿고, 그때 **fast-restore가 작동**합니다. 로그에서 주기적인 0V
강하가 보이는 게 정확히 이 현상입니다.

**그래서 대책이 달라집니다.** 전압 분배가 원인이라면 더 좋은 증폭기로 바꿔야 하지만,
실제 원인이 위 넷이라면 **전극 접촉을 좋게 하고 리드선을 짧게 하는 것**이 먼저입니다.
젤·구리테이프·짧은 악어클립이 바로 그 대책입니다.

##### 나중에 증폭기를 바꾼다면

고를 때 볼 것은 **입력 임피던스가 아니라 입력 바이어스 전류**입니다. 위 표에서 봤듯
문제를 만드는 건 그 전류이기 때문입니다.

| | AD8232 (지금) | AD8220 |
|---|---|---|
| 입력 방식 | — | JFET |
| 입력 바이어스 전류 | 50 pA(typ) / **1 nA(max)** | **10 pA(max)** |
| 전극 1 GΩ에서 오프셋 | 25 mV | **10 mV** |
| 대역 | 0.5–40 Hz 고정 | 제한 없음 (DC 결합 가능) |

여기에 **이득을 1100배에서 10~100배로 낮추면** 허용 입력 범위가 그만큼 넓어집니다.
단전원으로 만든다면 전극 하나로 식물을 전원 중간 전위에 바이어스하면 됩니다.

**지금 당장 필요한 건 아닙니다.** 젤·구리테이프로 접촉을 개선하는 게 먼저고, 그래도
표류와 레일 포화가 계속되면 그때 고려하세요.
...
다만 "몇 초 기다려라"라고 정해 두면 조건마다 맞지 않습니다. 표면에 붙이면 금방
가라앉지만, **찔러 넣은 전극은 초기 탈분극이 수 분에 걸쳐 진행된다**고 보고됩니다.
그래서 시간을 정하는 대신 **기준점이 실제로 잠잠해졌는지를 재서** 판단합니다.

- 최소 20초는 무조건 기다리고, 그 뒤로는 **기준점 기울기가 30 mV/분 아래로 떨어질 때까지**
  최대 3분까지 더 기다립니다. 조건이 좋으면 20초에 넘어가고, 나쁘면 더 기다립니다.
- 3분을 기다려도 계속 움직이면 **전극이 마르고 있다는 뜻**이라 경고하고 진행합니다.
- 이 구간의 값은 저장하지 않습니다. `--settle 0` 으로 끄고, `--settle-max` 로 상한을 바꿉니다.
- 수집이 끝나면 구간 전체의 기울기를 재서 **분당 표류량**을 알려주고, 이 속도면 몇 분
  뒤에 레일에 닿는지 계산해 줍니다.

```
[sensor_control] 전극 안정화 대기 (최소 20초, 최대 180초)
        5초  기준점 1.021V  기울기  +272.0 mV/분
       10초  기준점 1.025V  기울기  +150.6 mV/분
       15초  기준점 1.027V  기울기   +37.1 mV/분
       20초  기준점 1.027V  기울기    +3.1 mV/분
[sensor_control] 기준점이 안정됐습니다 (20초 소요).
[sensor_control] 기준점 표류: 출력 +12.4 mV/분 (전극 입력 환산 +0.0113 mV/분, ...)
```

#### 먼저 해야 할 실험 — 호일 대조군 (돈 0원, 10분)

이 프로젝트에서 **아직 답하지 못한 가장 중요한 질문**은 "지금 잡히는 신호가 정말 식물에서
오는가"입니다. 잎을 건드릴 때 반응이 보여도, 그게 식물 반응인지 사람 손·배선 흔들림·전원
잡음인지 구분되지 않았습니다.

선행연구는 이 질문에 **도체 대조(sham)** 로 답합니다. 식물 대신 **알루미늄 호일**에 전극을
**똑같은 간격·배치**로 붙이고 같은 절차로 수집·학습해서, 분류 정확도가 어떻게 되는지 봅니다.
해당 연구에서는 이 조건의 정확도가 우연 수준으로 떨어졌고, 그것이 "유의미한 분류에는 살아
있는 식물이 필요하다"는 근거가 되었습니다.

**해보는 법**

1. 알루미늄 호일을 화분 옆에 펴 놓고, RA·LA를 식물에 붙였을 때와 **같은 간격**으로 붙입니다
2. RL은 그대로 흙에 꽂아 둡니다
3. 평소와 똑같이 `정상`·`자극`·`수분부족` 세 상태를 수집합니다
   (`자극` 조건에서는 호일을 같은 방식으로 건드립니다)
4. 변환 → 학습을 돌리고 정확도를 봅니다

**결과 읽는 법**

| 호일 대조군 정확도 | 뜻 |
|---|---|
| 33% 부근 (우연 수준) | ✅ 식물 데이터의 정확도는 식물에서 온 것 |
| 식물과 비슷하게 높음 | ❌ 그 정확도는 **식물이 아니라 측정계·환경**에서 온 것 |

두 번째가 나와도 실패가 아닙니다. **그게 바로 보고서에 쓸 수 있는 결과**입니다 — 오히려
"높은 정확도가 그 자체로는 아무것도 증명하지 않는다"는 이 연구의 핵심 논지를 실측으로
뒷받침합니다.

> 여유가 되면 **사체 대조**도 같이 하세요. 바싹 마른 나뭇가지를 같은 흙에 꽂고 똑같이
...
흙에** 두었습니다(은/염화은 접착 전극 + 전도성 젤). 기준 전극을 줄기에 둔 변형도
보고되어 있습니다.

이 프로젝트는 지금까지 **RA·LA를 같은 줄기에 3~5cm 간격으로** 붙여 왔습니다. 두 방식의
차이는 **전극 사이 거리**입니다. 계측 증폭기는 두 전극의 전위 *차이*만 증폭하므로,
가까이 붙일수록 두 전극이 같은 전위를 보게 되어 차이가 작아집니다.

- 잘 안 잡히면 **잎 두 장으로 벌려서** 한 번 재보세요. 선행연구가 그 배치로 분류에
  성공했습니다.
- 다만 AD8232의 입력 허용 범위가 좁으므로(아래 참고), 거리를 벌려 신호가 커지면
  이번에는 레일에서 잘릴 수 있습니다. 수집 후 로그의 **레일 검출** 줄을 꼭 확인하세요.

> 문헌은 **전극을 찔러 넣으면 표면에 붙였을 때보다 신호 진폭이 커진다**고 보고합니다.
> 큐티클을 우회하기 때문입니다. 다만 삽입은 상처 반응을 일으키므로 안정화 시간이
> 더 필요합니다.
...
| 창의 이 비율 이상이 **출력 한계에 붙어 있으면** 버림 | 0.20 | `--rail-frac` |
| 필터 후 표준편차가 이보다 작으면 신호 없음 | 0.0005 V | `--min-std` |
| 검사 자체를 끄기 | — | `--no-quality` |

##### 출력 한계는 짐작하지 않고 신호에서 찾습니다

위 `3.0 V` 같은 문턱값은 전원 전압에서 짐작한 값이라 **보드마다 실제 한계와 다릅니다.**
이 프로젝트에서 수집한 CSV 6개를 조사해 보니 전부 최댓값이 **2.57~2.60 V**, 최솟값이
**-0.59~-0.61 V** 로 일정했습니다. 이게 이 보드의 실제 출력 한계입니다.

그래서 문턱값 `3.0 V` 로는 **전 구간이 2.56~2.60 V에 붙어 있던 파일**(= 완전히 포화)이
146창 전부 통과했습니다. 그대로 학습하면 증폭기 포화를 식물 신호로 배웁니다.

지금은 신호에서 직접 찾습니다. 증폭기가 포화하면 출력이 한 값에 머무르므로 **파일의
극값 근처에 샘플이 몰립니다.** 그 몰림이 보이면 그 극값을 출력 한계로 인정하고,
창별로 거기에 붙은 비율을 재서 버립니다.

```
    [레일 검출] 하단 -0.61V / 상단 2.59V — 전체 샘플의 14.0%가 레일에 붙어 있습니다.
```

실측 6개 파일로 검증한 결과입니다.

| 파일 | 레일에 붙은 비율 | 사용 가능한 창 | 예전 코드 |
|---|---|---|---|
| 줄기 1차 · 초기 측정 | 레일 미검출 | 100% | 100% |
| 줄기 2차 | 14% | 100% | 100% |
| 토양 삽입 | 29% | 0% | 100% |
| **줄기 3차** | **100%** | **0%** | **100%** ← 놓치던 것 |
| GND 직결 | 86% | 13.7% | 16.4% |

포화되지 않은 파일은 극값을 한두 번만 스치므로 **아무것도 버리지 않습니다.**

#### 한 상태만 다시 변환할 때 주의 — 설정이 섞이면 경고합니다

관리자 패널의 변환 버튼은 상태별로 따로 누를 수 있습니다(`--only`). 편리하지만 위험이
하나 있습니다. **한 상태만 다시 변환하면 나머지 상태의 스펙트로그램은 이전 설정으로
만들어진 채로 남는데**, 필터 메타는 이번 실행 값으로 덮어써집니다. 즉 데이터셋 안에
서로 다른 전처리로 만든 이미지가 섞이는데 아무도 모르는 상태가 됩니다.

이건 이 보고서가 지적하는 문제와 정확히 같은 구조입니다 — 모델이 **상태 차이가 아니라
전처리 설정 차이**를 학습하게 됩니다.

그래서 상태별로 실제 사용한 설정을 `preprocess_meta.json`에 누적해 두고, 서로 다르면
변환할 때마다 경고합니다.

```
⚠️  상태마다 다른 설정으로 변환된 스펙트로그램이 섞여 있습니다.
     수분부족: 대역 0.5~20.0Hz, 노치 50Hz, 창 10.0초/2.0초
     자극:     대역 0.5~30.0Hz, 노치 50Hz, 창 10.0초/2.0초
     정상:     대역 0.5~20.0Hz, 노치 50Hz, 창 10.0초/2.0초
     이 상태로 학습하면 모델이 상태 차이가 아니라 '전처리 설정 차이'를 배울 수 있습니다.
     전체를 다시 변환하세요:  python preprocess.py   (--only 없이)
```

**이 경고가 보이면 학습하지 말고 전체를 다시 변환하세요.**
...
> **자료(파일) API 는 관리자만 접근할 수 있습니다.** 화면에서는 자료 탭이 관리자 패널
> 안에 있어 로그인 전에는 보이지 않는데, 예전에는 API 가 열려 있어 주소만 알면
> 원시 CSV·학습된 모델을 그대로 받아갈 수 있었습니다. 지금은 `/api/files`,
> `/api/files/download`, `/api/files/preview`, `/api/trash` 모두 로그인이 필요합니다.
> 실시간 화면(`/data`, `/api/sensors`)은 그대로 공개입니다.
...
윈도우를 **시간순으로 나누고**(앞부분 train / 뒷부분 test), 경계에서 겹치는 윈도우를 버려
train/test 구간이 시간적으로 겹치지 않게 합니다(`chronological_group_split`).

**몇 개를 버려야 하는지는 창 길이와 스텝에서 계산합니다.** 창 W초를 S초씩 밀면 거리가
`W/S` 미만인 윈도우끼리 겹치므로, 버릴 개수는 `ceil(W/S) - 1` 입니다.

| 설정 | 겹침 | 버리는 창 |
|---|---|---|
| 2초 창 / 1초 스텝 (예전) | 50% | 1개 |
| **10초 창 / 2초 스텝 (현재)** | **80%** | **4개** |

이 값을 1로 고정해 두면 창을 늘렸을 때 경계에서 겹침이 그대로 남아 **누수가 다시
생깁니다.** 실제로 창을 2초에서 10초로 바꾼 뒤 한동안 그 상태였습니다 — 이 보고서가
지적하는 "높은 정확도가 그 자체로는 아무것도 증명하지 않는다"는 문제가 코드 안에서
재현된 셈입니다.
...
## 절차 자체를 검증하는 검사 (`src/sanity_check.py`)

측정 결과가 아니라 **분석 절차**가 맞는지 검사합니다. 코드를 고친 뒤 한 번씩 돌리세요.
...
**① 분할 겹침 검사.** 슬라이딩 창은 서로 겹칩니다. 창 10초를 2초씩 밀면 한 창은 앞뒤
4개 창과 신호를 공유하므로, 학습 구간과 평가 구간 경계에서 4창을 버려야 같은 신호가
양쪽에 들어가지 않습니다(`ceil(창/스텝) - 1`). 버리는 개수가 모자라면 정확도가
실제보다 높게 나옵니다. 이 검사는 여러 창/스텝 조합에서 경계 간격을 직접 세어
겹침 여부를 확인합니다.

**② 우연 수준 검사.** 세 상태를 통계적으로 완전히 똑같이 만든 신호(같은 분산의 잡음 +
같은 60Hz 성분)를 만들어 전처리·학습을 그대로 돌립니다. 구분할 근거가 없는 입력이므로
3분류 우연 수준인 33% 부근이어야 정상이며, 크게 넘으면 절차가 입력과 무관하게 정확도를
만들어내고 있다는 뜻입니다.

이 검사가 필요한 이유는 **음성 대조군 실험(알루미늄 호일)이 성립하려면 먼저 절차가
결백해야 하기 때문**입니다. 호일에서 낮은 정확도가 나왔을 때 그것이 "신호가 없어서"라고
말하려면, 절차가 아무 입력에서나 높은 정확도를 만들어내지 않는다는 게 먼저 보장돼야
합니다.

> 두 검사는 역할이 다릅니다. ②만으로는 ①의 문제를 못 잡습니다. 잡음에는 외울 구조가
> 없어서 창이 겹쳐도 정확도가 오르지 않기 때문입니다. 실제로 창을 2초에서 10초로 바꿨을
> 때 버리는 창 수가 그대로 1개여서 겹침이 생겼는데, ②는 통과하고 ①만 잡아냈습니다.

현재 결과:

```
[OK ] 창 2.0초/스텝 1.0초 · guard 1창 → 경계 간격 500샘플 (필요 500샘플)
[OK ] 창 10.0초/스텝 2.0초 · guard 4창 → 경계 간격 2500샘플 (필요 2500샘플)
[OK ] 창 10.0초/스텝 1.0초 · guard 9창 → 경계 간격 2500샘플 (필요 2500샘플)
[OK ] 창 15.0초/스텝 3.0초 · guard 4창 → 경계 간격 3750샘플 (필요 3750샘플)
최고 정확도 0.3529 (우연 수준 0.3333, 허용 상한 0.55)
```
